...
Theorem andb_true : (andb true true) = true.
Proof. reflexivity. Qed.

Theorem andb_false : (andb true false) = false.
Proof. reflexivity. Qed.
...
Proof.
  ...
  destruct b.
  - destruct c.
    + rewrite <- andb_true. reflexivity.
    + rewrite <- andb_false. rewrite -> H. reflexivity.    
  - destruct c.
    + rewrite <- andb_false. rewrite -> andb_commutative.
      rewrite -> H. reflexivity.
    + reflexivity.
Qed.
...
Example test_bin_incr2 : (bin_to_nat (incr (A (B (B Z)))) = (S (S (S (S (S (S (S O)))))))).
Proof. simpl. reflexivity. Qed.
                          
Example test_bin_to_nat : (incr Z) = (B Z).
Proof. simpl. reflexivity. Qed.

Example test_bin_incrto2 : (bin_to_nat (incr (A (B (B Z)))) = (S (S (S (S (S (S (S O)))))))).
Proof. simpl. reflexivity. Qed.
